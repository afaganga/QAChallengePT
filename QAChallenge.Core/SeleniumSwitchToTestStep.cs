...
{
    public class SeleniumSwitchToTestStep : SeleniumTestStep, ITestStep
    {
        ...
        public override ITestStepResult Execute(IContext context)
        {
            ...
            var method = typeof(ITargetLocator).GetMethods()
                .Where(m => m.Name == Action)
                .Where(m => !m.GetParameters().Any() || (m.GetParameters().Count() == 1 && m.GetParameters().First().ParameterType == typeof(string)))
                .First();
            var parameters = method.GetParameters().Any() ? new[] { Data } : new object[] { };
            method.Invoke(switchTo, parameters);
            return null;
        }
    }
}
